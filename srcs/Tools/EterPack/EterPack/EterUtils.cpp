...
int EterPackCompress(UINT8 *pRawData, UINT32 iRawSize, UINT8 **pCompressedData, UINT32 *iCompressedSize)
{
	...
	int destBuffSize = LZ4_compressBound(iRawSize);

    *pCompressedData = (unsigned char *)calloc(destBuffSize, 1);

	*iCompressedSize = LZ4_compress_default((const char *)pRawData, (char *)*pCompressedData, iRawSize, destBuffSize);
