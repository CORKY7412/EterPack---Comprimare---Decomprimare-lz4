...
EterPack::EterPack(string szEPKName, bool bReadFile)
	: szEterPackFilename(szEPKName), aEterPackData(0), pLastEterData(0), iEterPackSize(0), iDataSize(0)
{
	if (bReadFile)
	{
		FastIO::FileRead(szEterPackFilename, "rb", (void **)&aEterPackData, &iEterPackSize);
	}

	vKey = g_default_pack_key;
}

EterPack::~EterPack()
{
	if (this->aEterPackData)
	{
		free(this->aEterPackData);
	}

	if (this->pLastEterData)
	{
		free(this->pLastEterData);
	}
}
...
UINT8 *EterPack::Decompress(EterPackHeader *pEpkHeader, UINT8 *pEterData)
{
	...
	iOutputLZO = LZ4_decompress_safe((const char *)pEterData, (char *)pOutputData, pEpkHeader->CompressedSize, pEpkHeader->DecompressedSize);
...
bool EterPack::PutPack(string szFilename, enum STORAGE_TYPE eType, UINT32 *iOutSize, UINT32 *iHashCRC32)
{
	UINT32 iFileSize = 0, iPaddedSize = 0;
	UINT8 *pFileData = 0;
	...
	case NO_COMPRESSION_NO_ENCODING:
		...
		FastIO::FileWrite(szEterPackFilename.c_str(), "ab", pPaddedData, iPaddedSize);
	...
	case LZO_COMPRESSION_NO_ENCODING:
		...
		FastIO::FileWrite(szEterPackFilename.c_str(), "ab", pEterPackData, iPaddedSize);
