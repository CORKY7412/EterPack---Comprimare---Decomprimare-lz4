...
EterIndex::EterIndex(string szFilename)
 : EIX_Header(0), EIX_Info(0), pItemsList(0), pDecryptedData(0), pDecompressedData(0), iIndex(0), iPointer(0), szEterPackFilename(szFilename)
{
	vKey = g_default_index_key;
}
...
bool EterIndex::LoadFile()
{
	...
	FastIO::FileRead(szEterPackFilename.c_str(), "rb", (void **)&pRawData, &iFileSize);
	if(pRawData == NULL || iFileSize == 0)              return false;
	
	if(!this->Decrypt(pRawData, vKey, iFileSize)) return false;
...
bool EterIndex::Decompress(void)
{
	...
	iOutSizeLZO = LZ4_decompress_safe((const char *)this->pDecryptedData + sizeof(UINT32), (char *)pDecompressedData, EIX_Header->CompressedSize, EIX_Header->DecompressedSize);
...
void EterIndex::WritePack(void)
{
    ...
    ************************************/
	...
	FastIO::FileWrite(szEterPackFilename.c_str(), "wb", pEterPackData, iEterPackSize);
	free(pEterPackData);
}
